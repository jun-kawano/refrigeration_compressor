...
# ==============================================================================
def calculate_max_delta_p(res, compressor, num_ciclos):
    """Calcula a variação máxima de pressão entre passos de tempo consecutivos."""
    t_start_last = (num_ciclos - 1) * (1.0 / compressor.freq)
    idx = np.where(res['tempo'] >= t_start_last)[0]
    delta_p = np.abs(np.diff(res['P'][idx]))
    return np.max(delta_p)

def calculate_mass_flow(res, num_ciclos, compressor):
    t_start_last = (num_ciclos - 1) * (1.0 / compressor.freq)
    idx = np.where(res['tempo'] >= t_start_last)[0]
...
def calculate_indicated_power(res, num_ciclos, compressor):
    t_start_last = (num_ciclos - 1) * (1.0 / compressor.freq)
    idx = np.where(res['tempo'] >= t_start_last)[0]
    p_cycle = res['P'][idx]
    v_cycle = res['V'][idx]
...
def calculate_volumetric_efficiency(mass_flow, P_e, T_suc, compressor):
    rho_suc = CP.PropsSI('D', 'P', P_e, 'T', T_suc, cfg.fluid)
    volumetric_efficiency = mass_flow / (rho_suc * compressor.V_swept * compressor.freq)
    return volumetric_efficiency

def calculate_isentropic_efficiency(mass_flow, indicated_power, P_e, P_c, T_suc):
    h_suc = CP.PropsSI('H', 'P', P_e,'T', T_suc,  cfg.fluid)
    s_suc = CP.PropsSI('S', 'P', P_e,'T', T_suc,  cfg.fluid)
    h_des_isen = CP.PropsSI('H', 'P', P_c, 'S', s_suc, cfg.fluid)
...
def calculate_cop(mass_flow, indicated_power, P_e, P_c, T_suc):
    h_cond = CP.PropsSI('H', 'P', P_c,'Q', 0.0,  cfg.fluid)
    h_evap = CP.PropsSI('H', 'P', P_e,'T', T_suc,  cfg.fluid)
    delta_h = h_evap - h_cond
    q_evap = mass_flow * delta_h
    cop = q_evap / indicated_power
    return cop

def sim_summary(res, titulo, num_ciclos, P_e, P_c, T_suc, compressor):
    mass_flow = calculate_mass_flow(res, num_ciclos, compressor)
    work_cycle = calculate_cycle_work(res, num_ciclos, compressor)
    indicated_power = calculate_indicated_power(res, num_ciclos, compressor)
    vol_eff = calculate_volumetric_efficiency(mass_flow, P_e, T_suc, compressor)
    ...
    max_delta_p = calculate_max_delta_p(res, compressor, num_ciclos)
    ...
    print(f"  COP: {cop:.2f}")
    print(f"  Delta P máximo no passo: {max_delta_p:.0f} Pa")
...
if __name__ == '__main__':
    ...
    if csv_A.exists():
        ...
        sim_summary(res_A, "Condicao A", num_ciclos, P_eA, P_c, T_suc, cfg.compressor_ativo)
    else:
    ...
    if csv_B.exists():
        ...
        sim_summary(res_B, "Condicao B", num_ciclos, P_eB, P_c, T_suc, cfg.compressor_ativo)
    else:
